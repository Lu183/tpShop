...
<div class="layout pa-to-10 fo-fa-ar">
    ...
    <div class="fr wi940">
            ...
            <div class="order-state-progress">
            	<ol style="margin-left:0;">
                        ...
                        <li class="<?php if($order_info[pay_time] > 0): ?>current<?php endif; ?>">
                            ...
                            <dl>
                                <dt>付款成功</dt>
                                <?php if($order_info[pay_time] > 0): ?><dd>
                        ...
                        <li class="<?php if($order_info[shipping_time] > 0): ?>current<?php endif; ?>">
                            ...
                            <dl>
                                <dt>卖家发货</dt>
                                <?php if($order_info[shipping_time] > 0): ?><dd>
                        ...
                        <li class="<?php if($order_info[confirm_time] > 0): ?>current<?php endif; ?>">
                            ...
                            <dl>
                                <dt>完成</dt>
                                <?php if($order_info[confirm_time] > 0): ?><dd>
                ...
                <div class="ma-0--1">
                    <div class="o-pro">
                        <table border="0" cellpadding="0" cellspacing="0">
                            <tbody>
                            <?php if(is_array($order_action)): $i = 0; $__LIST__ = $order_action;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$action): $mod = ($i % 2 );++$i;?><tr>
        ...
        <!-- 订单处理信息-end-->
        <!-- 物流信息-s -->
        <?php if(!empty($express["data"])): ?><div class="o-info o-inff">
            <div class="fl">
        ...
        <div class="order-track">
              <div class="track-list">
                <ul>
                  <?php if(is_array($express["data"])): foreach($express["data"] as $k=>$vo): if($k == 0): ?><li class="first"><i class="node-icon"></i><span class="time"><?php echo ($vo["time"]); ?></span><span class="txt"><?php echo ($vo["context"]); ?></span></li>
	                  <?php else: ?>
	                  <li><i class="node-icon"></i><span class="time"><?php echo ($vo["time"]); ?></span><span class="txt"><?php echo ($vo["context"]); ?></span></li><?php endif; endforeach; endif; ?>
                </ul>
              </div>
        </div><?php endif; ?>
        <!-- 物流信息-e -->
        ...
        <div class="merge-list">
            <div class="ma-0--1">
                <div class="o-pro">
                    <table border="0" cellpadding="0" cellspacing="0">
                        <tbody>
                        <?php if(is_array($order_info["goods_list"])): $i = 0; $__LIST__ = $order_info["goods_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td rowspan="1" class="col-operate">                                    
                                    <?php if(($order_info[return_btn] == 1) and ($good[is_send] == 1)): ?><p class="p-link"><a style="color:#999;" href="<?php echo U('Home/User/return_goods',array('order_id'=>$order_info['order_id'],'order_sn'=>$order_info['order_sn'],'goods_id'=>$good['goods_id'],'spec_key'=>$good['spec_key']));?>">申请退款</a></p><?php endif; ?>
                                    <?php if(($order_info["comment_btn"] == 1) and ($good['is_comment'] == 0)): ?><p class="p-link"><a href="<?php echo U('Home/User/comment');?>" target="_blank"><span>评价</span></a></p><?php endif; ?>                                    
                                </td>
                            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            ...
            <div class="order-action-area te-al-ri cl-bo">
                <div class="woypdbe sc-acti-list" style="background-color:#f5f5f5; padding:10px 0 20px 0">
                    <span class="p-subtotal-price">合计（含运费）：<b>¥&nbsp;&nbsp;<span class="vab" id="payableTotal"><?php echo ($order_info["order_amount"]); ?></span></b></span>
                </div>
            </div>
            <div class="woypdbe sc-acti-list pa-to-20">                    
                <?php if($order_info["cancel_btn"] == 1): ?><a class="Sub-orders gwc-qjs" onClick="cancel_order(<?php echo ($order_info["order_id"]); ?>)"><span>取消订单</span></a><?php endif; ?>
                <?php if($order_info["pay_btn"] == 1): ?><a class="Sub-orders gwc-qjs" href="<?php echo U('/Home/Cart/cart4',array('order_id'=>$order_info[order_id]));?>"><span>立即支付</span></a><?php endif; ?>
                <?php if($order_info["receive_btn"] == 1): ?><a class="Sub-orders gwc-qjs" onClick=" if(confirm('你确定收到货了吗?')) location.href='<?php echo U('Home/User/order_confirm',array('id'=>$order_info['order_id']));?>'"><span>收货确认</span></a><?php endif; ?>
            </div>
        </div>
        <!-- 商品清单-end -->
    </div>
